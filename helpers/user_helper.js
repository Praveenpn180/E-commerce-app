...
module.exports={
    ...
    addToWishlist:(proId,userId)=>{
        return new Promise(async(resolve,reject)=>{
            let item= await db.get().collection(collection.USER_COLLECTION).findOne({_id:objectid(userId),
   ...
   if(hmac==details['payment[razorpay_signature]']){
    ...
    resetPassword:(email,password)=>{
        return new Promise(async(resolve,reject)=>{
          ...
          console.log(Password);
          db.get().collection(collection.USER_COLLECTION).updateOne({'email':email},{$set:{
            'Password':Password
          }})
          resolve(response)
          console.log(response);
